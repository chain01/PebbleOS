...
def cmd_manifest(args):
    ...
    map_files = [n.abspath().replace("\\", "/") for n in resource_nodes]
    reso_files = []
    lines = [
    ...
    for resource_type, indices in sorted(by_type.items()):
        for start in range(0, len(indices), BATCH_SIZE):
            ...
            outputs = [
                reso_output(definitions[i], args.builddir).replace("\\", "/") for i in batch
            ]
            reso_files.extend(outputs)
            deps = sorted(
                {d.replace("\\", "/") for i in batch for d in definition_deps(definitions[i])}
            )
            outputs_str = "\n    ".join(f'"{o}"' for o in outputs)
            deps_str = "\n    ".join(f'"{d}"' for d in deps)
            index_str = " ".join(str(i) for i in batch)
            output_arg = os.path.abspath(args.output).replace("\\", "/")
            lines.append(
                f"add_custom_command(\n"
                f"  OUTPUT\n    {outputs_str}\n"
                f"  COMMAND ${{PYTHON_EXECUTABLE}} ${{PBL_RESOURCES_PY}} reso\n"
                f'          --manifest "{output_arg}" --index {index_str}\n'
                f"  DEPENDS\n    {deps_str}\n"
                f'    "{output_arg}"\n'
